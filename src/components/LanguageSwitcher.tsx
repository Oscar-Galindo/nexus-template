...
import { locales, localeNames, localeFlags, getPathWithLocale, removeLocaleFromPath } from '@/i18n';
      ...
      {isOpen && (
        <>
          ...
          <div className="absolute right-0 z-20 mt-2 w-48 bg-white border border-gray-200 rounded-lg shadow-lg">
            {locales.map((locale) => {
              const localePath = getPathWithLocale(cleanPath, locale);
              ...
              return (
                <a
                  key={locale}
                  href={localePath}
                  className={`flex items-center gap-3 px-4 py-3 text-sm hover:bg-gray-50 first:rounded-t-lg last:rounded-b-lg ${
